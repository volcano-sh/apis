/*
Copyright The Volcano Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/
// Code generated by applyconfiguration-gen. DO NOT EDIT.

package v1beta1

import (
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	v1alpha1 "volcano.sh/apis/pkg/client/applyconfiguration/batch/v1alpha1"
)

// ReservationSpecApplyConfiguration represents a declarative configuration of the ReservationSpec type for use
// with apply.
type ReservationSpecApplyConfiguration struct {
	SchedulerName *string                               `json:"schedulerName,omitempty"`
	MinAvailable  *int32                                `json:"minAvailable,omitempty"`
	Tasks         []v1alpha1.TaskSpecApplyConfiguration `json:"tasks,omitempty"`
	Queue         *string                               `json:"queue,omitempty"`
	Owners        []ReservationOwnerApplyConfiguration  `json:"owners,omitempty"`
	TTL           *v1.Duration                          `json:"ttl,omitempty"`
	Expires       *v1.Time                              `json:"expires,omitempty"`
}

// ReservationSpecApplyConfiguration constructs a declarative configuration of the ReservationSpec type for use with
// apply.
func ReservationSpec() *ReservationSpecApplyConfiguration {
	return &ReservationSpecApplyConfiguration{}
}

// WithSchedulerName sets the SchedulerName field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the SchedulerName field is set to the value of the last call.
func (b *ReservationSpecApplyConfiguration) WithSchedulerName(value string) *ReservationSpecApplyConfiguration {
	b.SchedulerName = &value
	return b
}

// WithMinAvailable sets the MinAvailable field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the MinAvailable field is set to the value of the last call.
func (b *ReservationSpecApplyConfiguration) WithMinAvailable(value int32) *ReservationSpecApplyConfiguration {
	b.MinAvailable = &value
	return b
}

// WithTasks adds the given value to the Tasks field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the Tasks field.
func (b *ReservationSpecApplyConfiguration) WithTasks(values ...*v1alpha1.TaskSpecApplyConfiguration) *ReservationSpecApplyConfiguration {
	for i := range values {
		if values[i] == nil {
			panic("nil value passed to WithTasks")
		}
		b.Tasks = append(b.Tasks, *values[i])
	}
	return b
}

// WithQueue sets the Queue field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the Queue field is set to the value of the last call.
func (b *ReservationSpecApplyConfiguration) WithQueue(value string) *ReservationSpecApplyConfiguration {
	b.Queue = &value
	return b
}

// WithOwners adds the given value to the Owners field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the Owners field.
func (b *ReservationSpecApplyConfiguration) WithOwners(values ...*ReservationOwnerApplyConfiguration) *ReservationSpecApplyConfiguration {
	for i := range values {
		if values[i] == nil {
			panic("nil value passed to WithOwners")
		}
		b.Owners = append(b.Owners, *values[i])
	}
	return b
}

// WithTTL sets the TTL field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the TTL field is set to the value of the last call.
func (b *ReservationSpecApplyConfiguration) WithTTL(value v1.Duration) *ReservationSpecApplyConfiguration {
	b.TTL = &value
	return b
}

// WithExpires sets the Expires field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the Expires field is set to the value of the last call.
func (b *ReservationSpecApplyConfiguration) WithExpires(value v1.Time) *ReservationSpecApplyConfiguration {
	b.Expires = &value
	return b
}
