...
type PodGroupSpecApplyConfiguration struct {
	...
	BunchPolicy       []BunchPolicySpecApplyConfiguration    `json:"bunchPolicy,omitempty"`
}
...
// WithBunchPolicy adds the given value to the BunchPolicy field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the BunchPolicy field.
func (b *PodGroupSpecApplyConfiguration) WithBunchPolicy(values ...*BunchPolicySpecApplyConfiguration) *PodGroupSpecApplyConfiguration {
	for i := range values {
		if values[i] == nil {
			panic("nil value passed to WithBunchPolicy")
		}
		b.BunchPolicy = append(b.BunchPolicy, *values[i])
	}
	return b
}
